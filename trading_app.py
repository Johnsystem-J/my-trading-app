...
def get_default_settings():
    default_pair_settings = {
        "current_price": 1.08550, "rsi_14_value": 40.0, 
...
def create_empty_journal_df():
     return pd.DataFrame(columns=["Date", "Pair", "Direction", "Entry", "Exit", "SL", "TP", "Lot_Size", "P/L (Pips)", "P/L ($)", "Outcome", "Reason", "Review"])

# --- ฟังก์ชันวิเคราะห์อัตโนมัติ (ใหม่) ---
...
# --- ส่วนหลักของแอป ---
st.set_page_config(layout="wide", page_title="Trading Pro+ Assistant")
...
def create_analysis_panel(pair_name):
    ...
    st.session_state.app_state["pair_settings"][pair_name] = {"current_price": current_price, "rsi_14_value": rsi_14_value, "raw_atr_value": raw_atr_value, "is_bullish_candle": is_bullish_candle, "is_bearish_candle": is_bearish_candle, "near_key_level": near_key_level}

    st.divider()
    with st.container(border=True):
        st.subheader("บทวิเคราะห์และแผนการเทรด")
        
        # ดึงผลวิเคราะห์จาก session state
        ...
        if not d1_analysis or not h4_analysis:
            ...
            buy_d1_ok = (d1_analysis["trend"] == "ขาขึ้น (Uptrend)")
            buy_h4_ok = (h4_analysis["trend"] == "ขาขึ้น (Uptrend)")
            buy_structure_ok = h4_analysis["structure_ok"]
            buy_keylevel_ok = near_key_level
            buy_rsi_ok = (30 < rsi_14_value <= 45)
            buy_candle_ok = is_bullish_candle

            sell_d1_ok = (d1_analysis["trend"] == "ขาลง (Downtrend)")
            sell_h4_ok = (h4_analysis["trend"] == "ขาลง (Downtrend)")
            sell_structure_ok = h4_analysis["structure_ok"]
            ...
            c1, c2, c3 = st.columns(3)
            c1.markdown(f"{'✅' if buy_d1_ok or sell_d1_ok else '❌'} **D1 Trend:** {d1_analysis['trend']}")
            c1.markdown(f"{'✅' if buy_h4_ok or sell_h4_ok else '❌'} **H4 Trend:** {h4_analysis['trend']}")
            c2.markdown(f"{'✅' if buy_structure_ok or sell_structure_ok else '❌'} **H4 Market Structure:** {'สอดคล้อง' if h4_analysis['structure_ok'] else 'ไม่สอดคล้อง'}")
